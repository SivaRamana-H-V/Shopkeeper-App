...
import 'package:shopkeeper_app/shopkeeper/controllers/ledger_controller.dart';
...
class LedgerView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isShopkeeper = AuthSession.role != 'customer';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if (!isShopkeeper && status == EntryStatus.pending) ...[
              const Divider(),
              Row(
                ...
                children: [
                  TextButton(
                    onPressed: () {
                      ref
                          .read(ledgerControllerProvider(customerId).notifier)
                          .payEntry(id);
                    },
            ...
            if (!isShopkeeper && status == EntryStatus.disputed) ...[
              // Maybe allow re-approving? For now leave as is.
            ],
            if (!isShopkeeper && status == EntryStatus.approved) ...[
              // Maybe allow paying if not paid?
              // Assuming 'Approved' means approved by customer but not yet paid?
              // Or 'Approved' is final state?
              // Prompt says "Pay Now". If I pay, status becomes 'Paid' (new status).
              // So if approved, I can still pay.
              const Divider(),
              Row(
                mainAxisAlignment: MainAxisAlignment.end,
                children: [
